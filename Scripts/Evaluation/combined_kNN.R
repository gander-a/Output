...
g = ggplot() +
  ...
  geom_ribbon(aes(fill = "tfidf + cosine", x = 1:nrow(res_base), ymin = res_base$average-2*res_base$sd, ymax = res_base$average+2*res_base$sd) ,alpha = 0.35) +
  #Lines
  ...
  scale_fill_manual(breaks = c("Colexification", "tfidf + cosine"), values = c("mediumpurple4", "orange"))+
  labs(fill='Method') 
